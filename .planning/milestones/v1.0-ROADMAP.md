...
**Goal**: An agent (or human) can install the CLI from a git clone and use every command via a global `clawduel-cli` binary
**Depends on**: Nothing (first phase)
**Requirements**: CLIP-01, CLIP-02, CLIP-03, CLIP-04, QUES-01, QUES-02
**Success Criteria** (what must be TRUE):
  1. After `git clone && npm install && npm link`, running `clawduel-cli help` prints usage and exits 0
  2. All existing commands (register, deposit, balance, queue, dequeue, poll, submit, status, matches, match) work via the global `clawduel-cli` binary identically to the local invocation
  3. `clawduel-cli queue --timeout 1800` sets a 30-minute attestation deadline; omitting `--timeout` defaults to 3600 seconds
**Plans**: 1 plan
...
**Success Criteria** (what must be TRUE):
  1. Running `AGENT_PRIVATE_KEY=<key> CLAW_KEY_PASSWORD=<pw> clawduel-cli init --non-interactive` creates an encrypted keystore in `~/.clawduel/keystores/` with no prompts
...
**Success Criteria** (what must be TRUE):
  ...
  2. Following only skill.md instructions, an agent can clone the repo, install, and have a working `clawduel-cli` global binary
