...
human_verification:
  - test: "Fetch skill.md as a Claude Code agent and follow bootstrap section"
    expected: "claw-cli help prints usage and exits 0 after git clone / npm install / npm link"
...
| 2 | An agent following skill.md can install the CLI via git clone, npm install, npm link | VERIFIED | Lines 15-26: Bootstrap section with exact commands and `sudo npm link` fallback for permission errors. Verify step: `claw-cli help` |
...
|------|----|-----|--------|---------|
| skill.md bootstrap section | claw-cli global binary | `git clone`, `npm install`, `npm link`, `claw-cli help` | WIRED | Lines 17-26: exact bootstrap commands present, `npm link` documented, fallback `sudo npm link` present |
| skill.md fight loop section | CLI commands | exact claw-cli commands for queue, poll, submit | WIRED | Lines 82-92 (fight loop) and 125-138 (command reference) contain `claw-cli queue`, `claw-cli poll`, `claw-cli submit` |
...
| SKIL-03 | 03-01-PLAN.md | Complete fight loop with exact CLI commands per step | SATISFIED | Lines 73-92: 9-step per-match loop; one-time setup at lines 75-78; all commands use global `claw-cli` binary |
...
**Test:** In a clean environment (no existing claw-cli install), follow the Bootstrap section verbatim
**Expected:** `claw-cli help` prints usage text and exits 0 after completing npm link
**Why human:** Requires an isolated shell environment; cannot verify path registration via grep

#### 2. Keystore creation end-to-end

**Test:** Run `AGENT_PRIVATE_KEY=0x<testkey> CLAW_KEY_PASSWORD=testpass claw-cli init --non-interactive`
**Expected:** File created at `~/.clawduel/keystores/<address>.json` with 0600 permissions; subsequent `claw-cli balance` decrypts non-interactively
...
No gaps found. All 5 must-have truths are verified, all 10 requirements are satisfied, all 3 key links are wired, and the artifact is substantive (139 lines, non-placeholder). The document uses the correct agentskills.io spec structure (`name`/`description` at top level, `version`/`homepage` inside `metadata` block), uses the global `claw-cli` binary throughout, and contains no stale `npx tsx` references.
