...
**Primary recommendation:** Add `claw-cli.ts` to the TypeScript compilation (update `tsconfig.json` to include it), add a `bin` field to `package.json` pointing to `dist/claw-cli.js`, update the shebang to `#!/usr/bin/env node`, and add `--timeout` as an optional argument to the `queue` command.
...
|----|-------------|-----------------|
| CLIP-01 | `package.json` has `bin` field mapping `claw-cli` to compiled CLI entry point | Standard npm bin field pattern; point to `dist/claw-cli.js` |
| CLIP-02 | After `git clone && npm install && npm link`, `claw-cli` is available as global command | npm link creates symlink from global bin to local package bin entry |
| CLIP-03 | `claw-cli help` prints usage information and exits 0 | Help already works; need to verify exit code is 0 (it is -- returns from main naturally) |
| CLIP-04 | All existing commands work via global `claw-cli` binary | Requires claw-cli.ts to compile to JS and run under `node` instead of `tsx` |
...
- `tsconfig.json` changes to include `claw-cli.ts` in compilation
- `package.json` changes to add `bin` field
- Shebang change in `claw-cli.ts`
...
| Compiling claw-cli.ts via tsconfig | Keeping tsx shebang | tsx adds runtime dep and startup latency; compiled JS is the standard for npm bin entries |

## Architecture Patterns

### Compilation Strategy

**Critical finding:** `claw-cli.ts` is at the project root. `tsconfig.json` has `rootDir: ./src` and `include: ["src/**/*"]`. This means `claw-cli.ts` is NOT compiled today.
...
- `claw-cli.ts` compiles to `dist/claw-cli.js`
...
- Add `tsconfig.cli.json` that extends it and includes `claw-cli.ts`
...
{
  "bin": {
    "claw-cli": "dist/claw-cli.js"
  }
}
```

After `npm link`, this creates a symlink: `$(npm prefix -g)/bin/claw-cli` -> `<project>/dist/claw-cli.js`.
...
- **Hardcoding paths in help text:** The help text currently says `npx tsx claw-cli.ts <command>`. This must update to `claw-cli <command>`.
...
**What goes wrong:** `dist/claw-cli.js` has no shebang, so `claw-cli` command fails with a syntax error or tries to run as shell script.
**Why it happens:** Some TypeScript versions or configurations strip shebangs.
**How to avoid:** After building, verify `head -1 dist/claw-cli.js` shows `#!/usr/bin/env node`. TypeScript 5.3 should preserve it, but verify.
**Warning signs:** `claw-cli: line 1: syntax error` when running the global command.

### Pitfall 3: npm link Without Building First
**What goes wrong:** `npm link` symlinks to `dist/claw-cli.js` which doesn't exist yet, resulting in `ENOENT` or stale code.
...
### Pitfall 4: File Permission on dist/claw-cli.js
...
**How to avoid:** npm link handles permissions when a `bin` field is present. If issues arise, add `chmod +x dist/claw-cli.js` to the build script.
**Warning signs:** `Permission denied` when running `claw-cli`.

### Pitfall 5: Help Text Still References Old Invocation
**What goes wrong:** After installing globally, `claw-cli help` still says `npx tsx claw-cli.ts <command>`.
**Why it happens:** The help text on line 837 and error messages on lines 906, 956 hardcode the old invocation.
**How to avoid:** Update all references from `npx tsx claw-cli.ts` to `claw-cli`.
...
| Full suite command | `npm run build && npm link && claw-cli help` |

### Phase Requirements -> Test Map
| Req ID | Behavior | Test Type | Automated Command | File Exists? |
|--------|----------|-----------|-------------------|-------------|
| CLIP-01 | package.json has bin field | smoke | `node -e "const p=require('./package.json'); if(!p.bin || !p.bin['claw-cli']) process.exit(1)"` | N/A (inline) |
| CLIP-02 | npm link makes claw-cli available | smoke | `npm run build && npm link && which claw-cli` | N/A (inline) |
| CLIP-03 | claw-cli help exits 0 | smoke | `claw-cli help; echo $?` | N/A (inline) |
| CLIP-04 | All commands work via binary | manual-only | Manual -- requires wallet, RPC, backend | N/A |
| QUES-01 | queue accepts --timeout flag | smoke | `grep -q "timeout" dist/claw-cli.js` | N/A (inline) |
| QUES-02 | Default timeout is 3600 | smoke | `grep -q "3600" dist/claw-cli.js` | N/A (inline) |

### Sampling Rate
- **Per task commit:** `npm run build` (must succeed)
- **Per wave merge:** `npm run build && npm link && claw-cli help`
...
- Project source code: `claw-cli.ts`, `package.json`, `tsconfig.json` -- direct inspection
