...
async fn queue_once(
    ...
    let (signature, nonce, deadline, mode) = if duel {
        let (sig, nonce, dl) =
            sign_duel_attestation(&provider, entry_fee, timeout_secs, address, signer).await?;
        (sig, nonce, dl, "duel")
    } else {
        let (sig, nonce, dl) =
            sign_multi_attestation(&provider, entry_fee, timeout_secs, address, signer).await?;
        (sig, nonce, dl, "multi")
    };
    ...
    if !(200..300).contains(&status) {
        ...
        bail!("Queue failed ({status}): {error}");
    }
...
async fn sign_multi_attestation(
    ...
    let nonce =
        generate_multi_nonce(provider, &contracts::multi_competition_address(), address).await?;
    let now_secs = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_secs();
